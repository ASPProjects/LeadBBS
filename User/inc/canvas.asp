...
Public Sub DrawTextWE(ByVal lX,ByVal lY,sText,space)
  ...
  dim sTemp,lOffset,lXa,lYa,lValue,tt
	...
	Dim RndData,RndCol
	Redim RndData(t2),RndCol(t2),widthData(t2),WData(t2)
	For iTemp1 = 1 to t2
		Randomize
		...
		Else
			widthData(Itemp1-1) = widthData(Itemp1-1-1) + WData(iTemp1-1)+space
		End If
	Next
	...
	For iTemp1 = 0 to t1
		...
		For iTemp2 = 1 to t2
			...
			If inStr(t7,"1") > 0 Then
				...
				t3=WData(iTemp2-1)
				For iTemp3 = 1 to t3
					bChar = Mid(t7,iTemp3,1)
					if bChar <> "0" then
						'Pixel(t4 + iTemp3,t6) = CLng(bChar)
...
' ***************************************************************************
 ...
 ' response.End
  ...
  DIM  sTempData_LEN
  sTempData_LEN = LenB(sTempData)
  for iTemp = 1 to sTempData_LEN step 255
 ...
 private function GetGlobalColourTable()
  ...
  dim g_ubound
  g_ubound = UBound(GlobalColourTable) - 1
  for iTemp = 0 to g_ubound
 ...
 public function DecodeBMP(sBuffer)
  ...
  if MidB(sBuffer,1,2) = BitmapMagicNumber then
   ...
   if lBPP = 8 and lCompression = 0 then
    ...
    dim L_sBitmap
    L_sBitmap = LenB(sBitmap)
    For lTemp = 1 to L_sBitmap Step lLineSize
