(function () {
	...
	var Calendar = {
		...
		saveFun : function (d, sucFun, failFun) {
			$.ajax({
				url : "/script/note.js",
				data : d,
				type : "post",
				...
				success : function (d) {
					...
					if (d && d.err == 0) {
						...
						failFun && failFun((d && d.msg) || "保存失败!");
					}
				}
			});
		},
		getFun : function (d, sucFun, failFun) {
			var cd = $.extend({
					type : "search_note"
				}, d);
			$.ajax({
		...
		delFun : function (d, sucFun, failFun) {
			$.ajax({
				url : "scripts/note.js",
				data : $.extend({
					type : "delete_nonte"
				...
				success : function (d) {
					if (d && d.err == 0) {
						...
						failFun && failFun(d && d.msg);
					}
				}
			});
		},
		updateFun : function (d, sucFun, failFun) {
			$.ajax({
				url : "scripts/note.js",
				data : $.extend({
					type : "edit_nonte"
	...
	$(".a_5").click(function (e) {
		...
		try {
			if (!$("#recordDateSel").datepicker) {
				$.getScript("date.min.js", function () {
	...
	window.getRecordByM = getRecordByM;
	function getRecordByM(dateM, n) {
		$(".js").remove();
		setTimeout(function () {
			Calendar.getFun({
				date_type : n || "",
				date_ymd : dateM || ($("#current-data").text() + "-" + $("#current-data-month").text() + "-1"),
				uid : window.Note.uid || ""
			}, function (d) {
				var d = curMonthRecordList = d,
				c = cld,
				tbl = $(".calendar td");
				for (var i = 0, l = c.length + 1; i < l; i++) {
					var td = tbl.eq(i + c.firstWeek - 1);
					if (d && d.err == 0 && d.data[i] && td.find(".js").length == 0) {
						td.find(">div").append('<b class="js" title="点击查看记事" data-day="' + i + '"></b>');
					}
				}
			});
		}, 400);
	}
	function getRecordByMList(dateM, n) {
		var dateM = dateM || /\d{4}\-\d{1,2}\-\d{1,2}/.exec($("#info_all").text())[0],
		n = n || "";
		Calendar.getFun({
			date_type : n,
			date_ymd : dateM,
			...
			curMonthRecordList = d;
			var argD = /\d{4}\-(\d{1,2})\-(\d{1,2})/.exec(dateM);
			showRecordList(argD[2], argD[1]);
		}, function () {
			showPageFun("Date");
			getRecordByM();
		});
	...
	var getNong = function () {
		...
		$.ajax({
			url : "scripts/action.js",
			type : "post",
			data : "date=" + to_day.jin + "&type=load",
			cache : false,
			success : function (data) {
				try {
					var data = eval("(" + data + ")"),
						list_good = "",
						list_bad = "";
					if (data.good != 500 && data.good != '500') {
						for (var i = 0; i < data.good.length; i++) {
							list_good += "<span>" + data.good[i] + "</span>";
						}
					}else{
						list_good = "<span>暂无</span>";
					}
					if (data.bad != 500 && data.bad != '500') {
						for (var i = 0; i < data.bad.length; i++) {
							list_bad += "<span>" + data.bad[i] + "</span>";
						}
					}else{
						list_bad = "<span>暂无</span>";
					}
					$("#yi").html(list_good);
					$("#ji").html(list_bad);
				} catch (ex) {
					$("#yi").html("<span>暂无</span>");
					$("#ji").html("<span>暂无</span>");
					return;
				}
				getTimetoDom(data.times);
				if (to_day.jin) {
					$("#inYear").val(to_day.jin.split("-")[0]);
					$("#inMonth").val(to_day.jin.split("-")[1]);
					$("#inData").val(to_day.jin.split("-")[2]);
					$("#inYear2").val(to_day.jin.split("-")[0]);
					$("#inMonth2").val(to_day.jin.split("-")[1]);
					$("#inData2").val(to_day.jin.split("-")[2]);
				} else {
					setTimeout(function () {
						if (to_day.jin) {
							$("#inYear").val(to_day.jin.split("-")[0]);
